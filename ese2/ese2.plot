...
set output "output_data/nice_neutron_TOV.tex"
set title "$\\theta$ for stable values fo $n$ TOV"
set xlabel "$x$"
set ylabel "$\\theta_n(x)$"
# set yrange [-1e2:1e6]
set autoscale
plot "output_data/neutron_TOV_n_3.00.csv" using 1:2 with lines title "$n$: $3.00$"
set output
!pdflatex -output-directory=output_data output_data/nice_neutron_TOV.tex

set output "output_data/nice_neutron_mass_TOV.tex"
set title "$m$ for stable values fo $n$ TOV"
set xlabel "$x$"
set ylabel "$m(x)$"
set autoscale
# set yrange [-1e2:1e10]
plot "output_data/neutron_TOV_n_3.00.csv" using 1:3 with lines title "$n$: $3.00$"
set output
!pdflatex -output-directory=output_data output_data/nice_neutron_mass_TOV.tex
